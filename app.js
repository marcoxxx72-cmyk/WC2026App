      ...
      tab===8&&e('div',{style:{paddingBottom:16}},
        ...
        e('div',{style:{marginBottom:20}},
          ...
          e('div',{style:{position:'relative',width:'100%',maxWidth:300,margin:'0 auto 12px',height:130,background:'linear-gradient(180deg,#1a4a1a,#2d7a2d)',borderRadius:10,overflow:'hidden',border:'2px solid rgba(255,255,255,0.2)'}},
            ...
            e('div',{style:{position:'absolute',top:keeperDir==='left'?15:keeperDir==='right'?15:22,left:keeperDir==='left'?'21%':keeperDir==='right'?'55%':'38%',fontSize:30,transition:'all 0.4s ease',filter:shotResult==='goal'?'grayscale(0.5)':''}},'🧤'),
          ...
          // Buttons
          gamePhase==='idle'&&e('button',{onClick:function(){setGamePhase('shooting');},style:{width:'100%',background:'linear-gradient(135deg,'+G+',#b8963e)',border:'none',borderRadius:10,padding:'12px 0',fontSize:14,fontWeight:'bold',color:'#0a0a1a',cursor:'pointer'}},
            lang==='fr'?'⚽ Commencer':lang==='es'?'⚽ Empezar':lang==='pt'?'⚽ Comecar':lang==='it'?'⚽ Inizia':lang==='de'?'⚽ Starten':'⚽ Start'
          ),
          gamePhase==='shooting'&&e('div',null,
            e('div',{style:{fontSize:11,color:'#6a86a0',textAlign:'center',marginBottom:8}},
              lang==='fr'?'Ou tirez-vous ?':lang==='es'?'Donde tiras ?':lang==='pt'?'Para onde ?':lang==='it'?'Dove tiri ?':lang==='de'?'Wohin ?':'Where do you shoot?'
            ),
            e('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}},
              ['left','center','right'].map(function(dir){
                var labels={left:{en:'⬅ Left',fr:'⬅ Gauche',es:'⬅ Izq.',pt:'⬅ Esq.',it:'⬅ Sinistra',de:'⬅ Links'},center:{en:'⬆ Center',fr:'⬆ Centre',es:'⬆ Centro',pt:'⬆ Centro',it:'⬆ Centro',de:'⬆ Mitte'},right:{en:'➡ Right',fr:'➡ Droite',es:'➡ Der.',pt:'➡ Dir.',it:'➡ Destra',de:'➡ Rechts'}};
                return e('button',{key:dir,onClick:function(){shootPenalty(dir);},style:{background:'linear-gradient(135deg,rgba(10,30,80,0.9),rgba(20,50,120,0.9))',border:'2px solid '+G,borderRadius:9,padding:'13px 4px',fontSize:13,fontWeight:'bold',color:G,cursor:'pointer'}},labels[dir][lang]||labels[dir].en);
              })
            )
          ),
          gamePhase==='animating'&&e('div',{style:{textAlign:'center',color:G,padding:10,fontSize:13}},'...'),
          gamePhase==='done'&&e('div',{style:{textAlign:'center'}},
            e('div',{style:{fontSize:36,marginBottom:6}},gameScore>=4?'🏆':gameScore>=3?'⭐':gameScore>=2?'👍':'😢'),
            e('div',{style:{fontSize:20,fontWeight:'bold',color:G,marginBottom:10}},gameScore,' / 5'),
            e('div',{style:{fontSize:11,color:'#9bb0c8',marginBottom:12}},
              gameScore===5?'PERFECT !':gameScore>=4?'Excellent !':gameScore>=3?'Not bad !':'Keep trying !'
            ),
